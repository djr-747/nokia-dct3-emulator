...
// DCT3 addition: ARM7 FIQ entry (the vendored upstream only exposed IRQ). The
// MAD2 firmware drives the MBUS UART (and sleep clock) via FIQ, so the platform
// needs to deliver them. Mirrors ARMRaiseIRQ but enters MODE_FIQ at BASE_FIQ and
// masks both F and I. See third_party/mgba-arm/README for the vendored deltas.
