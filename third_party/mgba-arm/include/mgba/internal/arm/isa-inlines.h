...
static inline int32_t ARMWritePC(struct ARMCore* cpu) {
	...
	cpu->gprs[ARM_PC] = pc;
	// DCT3 fork: ARM7TDMI branch / PC-write = 2S+1N. The per-instruction prefetch base
	// is already charged by the caller (ARM_PREFETCH_CYCLES), so the pipeline-refill
	// constant here is 1, not stock mGBA's 2 (which over-counted every branch by a cycle).
	return 1 + cpu->memory.activeNonseqCycles32 + cpu->memory.activeSeqCycles32;
}
...
static inline int32_t ThumbWritePC(struct ARMCore* cpu) {
	...
	cpu->gprs[ARM_PC] = pc;
	// DCT3 fork: see ARMWritePC — refill constant 1 (was 2) so a taken Thumb branch is
	// 2S+1N = 3 cycles, not 4.
	return 1 + cpu->memory.activeNonseqCycles16 + cpu->memory.activeSeqCycles16;
}
