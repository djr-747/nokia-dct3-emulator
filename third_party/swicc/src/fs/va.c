    ...
    if (ret == SWICC_RET_SUCCESS)
    {
        ...
        if (ret == SWICC_RET_SUCCESS)
        {
            ...
            case SWICC_FS_ITEM_TYPE_FILE_EF_CYCLIC: {
                 ...
                 * always changes even for selections using SID.
                 */
                swicc_fs_file_st const file_adf = fs->va.cur_adf;
                swicc_disk_tree_st *const tree_adf = fs->va.cur_tree_adf;
                memset(&fs->va, 0U, sizeof(fs->va));
...
/* LOCAL PATCH (see third_party/swsim/README.md): current-DF-scoped SELECT-by-FID
 * helper. Matches an immediate child of the current DF by file-id. */
typedef struct va_selid_curdf_ud_s
{
    swicc_fs_id_kt fid;
    bool found;
    swicc_fs_file_st file;
} va_selid_curdf_ud_st;
static swicc_ret_et va_selid_curdf_cb(swicc_disk_tree_st *const tree,
                                      swicc_fs_file_st *const file,
                                      void *const userdata)
{
    (void)tree;
    va_selid_curdf_ud_st *const ud = userdata;
    if (file->hdr_file.id == ud->fid)
    {
        ud->found = true;
        ud->file = *file;
        return SWICC_RET_ERROR; /* stop iteration on first match */
    }
    return SWICC_RET_SUCCESS;
}
                                     ...
                                     swicc_fs_id_kt const fid)
{
    swicc_disk_tree_st *tree;
    swicc_fs_file_st file;
    /* LOCAL PATCH (see third_party/swsim/README.md): resolve SELECT-by-FID
     * CURRENT-DF-FIRST per GSM 11.11 / ISO 7816-4. The stock code went straight to
     * a disk-GLOBAL id lookup (swicc_disk_lutid_lookup below), which ignores the
     * selected DF: when the same file-id exists under several DFs (e.g. EF_LOCI
     * 6F7E under DF_GSM 7F20, DF_DCS1800 7F21 and 7F40), the global LUT collapses
     * them to one entry and serves the wrong DF's copy. Search the children of the
     * currently-selected DF first; only fall back to the global lookup (unchanged
     * behaviour, covers MF / parent / cross-DF selects) when not found here. */
    if (fs->va.cur_tree != NULL)
    {
        va_selid_curdf_ud_st ud = {.fid = fid, .found = false};
        swicc_disk_file_foreach(fs->va.cur_tree, &fs->va.cur_df,
                                va_selid_curdf_cb, &ud, false);
        if (ud.found)
        {
            return va_select_file(fs, fs->va.cur_tree, ud.file);
        }
    }
