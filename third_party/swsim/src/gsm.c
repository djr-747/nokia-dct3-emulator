                       ...
                       uint16_t *const buf_res_len)
{
    ...
    case SWICC_FS_ITEM_TYPE_FILE_EF_CYCLIC: {
        ...
        /* LOCAL PATCH: initialise here (0 for a transparent EF, which has no
         * record-length byte). Upstream declared this uninitialised and then
         * clobbered it to 0 for ALL EFs just before emitting the FCP (see the
         * removed `rcrd_length = 0;` below), so record EFs reported a bogus
         * record length of 0. */
        uint8_t rcrd_length = 0;
        ...
        /* LOCAL PATCH: upstream had `rcrd_length = 0;` here, which clobbered the
         * record length computed above (rcrd_length = rcrd_size) for every
         * linear-fixed / cyclic EF. That made the GSM SELECT FCP report record
         * length 0 (byte 14) for record EFs, so the Nokia 3310's record-EF /
         * SIM-storage init (phonebook EF_ADN 6F3A, SMS EF_SMS 6F3C, MSISDN
         * 6F40, ...) could never build its record cache and the SIM never
         * reached full storage readiness ("SIM not ready"). Removed so the
         * real record length is emitted. */
