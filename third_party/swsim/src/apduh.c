...
/**
 * @brief Handle the READ RECORD command in the proprietary class A0 of
 * GSM 11.11.
 * @note As described in GSM 11.11 v4.21.1 (ETS 300 608) clause.9.2.4 (command),
 * clause.9.3 (coding), and 9.4 (status conditions).
 * @note LOCAL PATCH (not upstream): upstream swSIM never wired GSM READ RECORD
 * (A0 B2) — only READ BINARY (A0 B0) — so record-structured EFs (e.g. EF_ACM
 * 6F39, a cyclic file) answered A0 B2 with 6D00 and pre-UICC 2G phones (Nokia
 * 3310) looped forever in their Advice-of-Charge/ACM read. This handler mirrors
 * the ISO interindustry READ RECORD (apduh_rcrd_read in swicc) but uses the GSM
 * response convention (immediate data + 90 00) and supports P1=0x00
 * "current record" (absolute/current mode), which the firmware relies on.
 */
static swicc_apduh_ft apduh_gsm_rcrd_read;
static swicc_ret_et apduh_gsm_rcrd_read(swicc_st *const swicc_state,
                                        swicc_apdu_cmd_st const *const cmd,
                                        swicc_apdu_res_st *const res,
                                        uint32_t const procedure_count)
{
    (void)procedure_count;
    ...
    swicc_fs_file_st *const file = &swicc_state->fs.va.cur_file;
    /* GSM 11.11 clause.8 table.8: READ RECORD only works on record EFs. */
    if (file->hdr_item.type != SWICC_FS_ITEM_TYPE_FILE_EF_LINEARFIXED &&
        file->hdr_item.type != SWICC_FS_ITEM_TYPE_FILE_EF_CYCLIC)
    {
        res->sw1 = (file->hdr_item.type == SWICC_FS_ITEM_TYPE_INVALID)
                       ? 0x94  /* "No EF selected." */
                       : 0x94; /* "File is inconsistent with the command." */
        res->sw2 = (file->hdr_item.type == SWICC_FS_ITEM_TYPE_INVALID) ? 0x00
                                                                       : 0x08;
        res->data.len = 0U;
        return SWICC_RET_SUCCESS;
    }
    ...
    if (swicc_disk_file_rcrd_cnt(swicc_state->fs.va.cur_tree, file,
    ...
    /* Resolve the target record index (0-based) from P1 (record number,
     * 1-based; 0 = current record) and P2 (mode). */
    uint8_t const p1 = cmd->hdr->p1;
    uint8_t const mode = cmd->hdr->p2;
    uint32_t rcrd_idx;
    switch (mode)
    {
    case 0x02: /* Next record, relative to the current one. */
        rcrd_idx = (uint32_t)swicc_state->fs.va.cur_rcrd.idx + 1U;
        break;
    case 0x03: /* Previous record, relative to the current one. */
        if (swicc_state->fs.va.cur_rcrd.idx == 0U)
        {
            res->sw1 = SWICC_APDU_SW1_CHER_P1P2;
            res->sw2 = 0x00;
            res->data.len = 0U;
        ...
        rcrd_idx = (uint32_t)swicc_state->fs.va.cur_rcrd.idx - 1U;
        break;
    case 0x04: /* Absolute/current mode: P1 record number, 0 = current. */
        rcrd_idx = (p1 == 0x00) ? (uint32_t)swicc_state->fs.va.cur_rcrd.idx
                                : (uint32_t)(p1 - 1U);
        break;
    default:
        res->sw1 = SWICC_APDU_SW1_CHER_P1P2;
    ...
    if (rcrd_idx >= rcrd_cnt)
    {
        res->sw1 = SWICC_APDU_SW1_CHER_P1P2; /* "out of range." */
        res->sw2 = 0x00;
        res->data.len = 0U;
        return SWICC_RET_SUCCESS;
    }
    ...
    uint8_t *rcrd_buf;
    uint8_t rcrd_len;
    if (swicc_disk_file_rcrd(swicc_state->fs.va.cur_tree, file,
                             (swicc_fs_rcrd_idx_kt)rcrd_idx, &rcrd_buf,
                             &rcrd_len) != SWICC_RET_SUCCESS)
    {
        res->sw1 = SWICC_APDU_SW1_CHER_UNK;
        res->sw2 = 0x00;
        res->data.len = 0U;
    ...
    /* GSM 11.11: P3 must equal the record length. */
    if (*cmd->p3 != rcrd_len)
    {
        res->sw1 = SWICC_APDU_SW1_CHER_LEN; /* "Incorrect parameter P3." */
        res->sw2 = 0x00;
    ...
    /* Update the current record pointer and return the record data. */
    swicc_va_select_record_idx(&swicc_state->fs,
                               (swicc_fs_rcrd_idx_kt)rcrd_idx);
...
/**
 * @brief Handle the UPDATE RECORD command in the proprietary class A0 of
 * GSM 11.11.
 * @note As described in GSM 11.11 v4.21.1 (ETS 300 608) clause.9.2.5 (command),
 * clause.9.3 (coding), and 9.4 (status conditions).
 * @note LOCAL PATCH (not upstream): upstream swSIM's GSM (CLA A0) demux wired
 * UPDATE BINARY (A0 D6) but never UPDATE RECORD (A0 DC), so writing a
 * record-structured EF — notably adding a contact to EF_ADN (6F3A, a
 * linear-fixed record EF) — fell through to 6D00 (INS not supported) and the
 * Nokia 3310 reported "SIM not ready" for the phonebook. This handler mirrors
 * the record ADDRESSING of apduh_gsm_rcrd_read (P1 record number 1-based,
 * 0 = current; P2 mode 0x04 absolute/current, 0x02 next, 0x03 previous) and the
 * T=0 data-in / procedure handshake of apduh_3gpp_bin_update, then writes the
 * incoming record via the swICC disk record buffer (swicc_disk_file_rcrd
 * returns a writable pointer into file->data, the same store UPDATE BINARY
 * writes through). PIN/CHV is disabled in this fixture so EF_ADN writes are
 * permitted, matching UPDATE BINARY.
 */
static swicc_apduh_ft apduh_gsm_rcrd_update;
static swicc_ret_et apduh_gsm_rcrd_update(swicc_st *const swicc_state,
                                          swicc_apdu_cmd_st const *const cmd,
                                          swicc_apdu_res_st *const res,
                                          uint32_t const procedure_count)
{
    swicc_fs_file_st *const file = &swicc_state->fs.va.cur_file;
    /* GSM 11.11 clause.8 table.8: UPDATE RECORD only works on record EFs. */
    if (file->hdr_item.type != SWICC_FS_ITEM_TYPE_FILE_EF_LINEARFIXED &&
        file->hdr_item.type != SWICC_FS_ITEM_TYPE_FILE_EF_CYCLIC)
    {
        res->sw1 = 0x94; /* "No EF selected" / "inconsistent with command". */
        res->sw2 = (file->hdr_item.type == SWICC_FS_ITEM_TYPE_INVALID) ? 0x00
                                                                       : 0x08;
        res->data.len = 0U;
        return SWICC_RET_SUCCESS;
    ...
    /* Resolve the target record index (0-based) from P1 (record number,
     * 1-based; 0 = current record) and P2 (mode). Same addressing modes as
     * READ RECORD; the firmware drives EF_ADN (linear-fixed) with absolute
     * mode (P2=0x04). */
    uint8_t const p1 = cmd->hdr->p1;
    uint8_t const mode = cmd->hdr->p2;
    uint32_t rcrd_idx;
    ...
    /* T=0 data-in handshake (mirrors apduh_3gpp_bin_update): on the first pass
     * no data has arrived yet — acknowledge and pull P3 bytes from the reader.
     * The handler is then re-entered with procedure_count>=1 and cmd->data
     * populated. */
    if (procedure_count == 0U)
    {
        if (cmd->data->len != 0U)
        ...
        if (*cmd->p3 > 0)
        {
            SWICC_APDUH_RES(res, SWICC_APDU_SW1_PROC_ACK_ALL, 0U, *cmd->p3);
            return SWICC_RET_SUCCESS;
        }
    }

    /* Resolve the record buffer (points into file->data) and its length. */
    uint8_t *rcrd_buf;
    uint8_t rcrd_len;
    if (swicc_disk_file_rcrd(swicc_state->fs.va.cur_tree, file,
                             (swicc_fs_rcrd_idx_kt)rcrd_idx, &rcrd_buf,
    ...
    /* GSM 11.11: P3 and the received data length must equal the record
     * length. */
    if (*cmd->p3 != rcrd_len || cmd->data->len != rcrd_len)
    {
        res->sw1 = SWICC_APDU_SW1_CHER_LEN; /* "Incorrect parameter P3." */
        res->sw2 = 0x00;
        res->data.len = 0U;
    ...
    /* Write the record into the disk store and update the current-record
     * pointer. */
                                          ...
                                          uint32_t const procedure_count)
{
    ...
    /* offset_hi/offset_lo are already decoded numeric APDU octets. Combining
     * them with shifts produces the host-order integer directly; applying
     * be16toh again turns 0x0004 into 0x0400 on every little-endian host and
     * makes valid partial EF_LOCI updates fail with 6A86. */
    uint16_t const offset = meth == METH_SFI
                                ? offset_lo
                                : (uint16_t)(((uint16_t)offset_hi << 8) |
                                             offset_lo);
    /* Perform requested operation. */
    {
                             ...
                             uint32_t const procedure_count)
{
    ...
    case SWICC_APDU_CLA_TYPE_PROPRIETARY:
        ...
        case 0x10: /* TERMINAL PROFILE */
            /* ETSI (CLA 0x80) + GSM 11.14 (CLA 0xA0) — LOCAL PATCH: pre-UICC 2G
             * phones (e.g. Nokia 3310) send TERMINAL PROFILE as A0 10; upstream
             * swSIM only wired the ETSI 80 10 class, so A0 10 fell through to 6D00
             * and the phone looped forever. The handler ignores the profile anyway. */
            if (cmd->hdr->cla.raw == 0x80 || cmd->hdr->cla.raw == 0xA0)
            {
        ...
        case 0xB2: /* READ RECORD */
            /* GSM — LOCAL PATCH: see apduh_gsm_rcrd_read. Upstream swSIM never
             * wired A0 B2, so record EFs (EF_ACM 6F39) looped at 6D00. */
            if (cmd->hdr->cla.raw == 0xA0)
            {
                ret =
                    apduh_gsm_rcrd_read(swicc_state, cmd, res, procedure_count);
            }
            break;
        case 0xF2: /* STATUS */
            /* ETSI + 3GPP */
        ...
        case 0xDC: /* UPDATE RECORD */
            /* GSM — LOCAL PATCH: see apduh_gsm_rcrd_update. Upstream swSIM wired
             * UPDATE BINARY (A0 D6) but never UPDATE RECORD (A0 DC), so writing
             * a record EF (adding a contact to EF_ADN 6F3A) fell to 6D00. */
            if (cmd->hdr->cla.raw == 0xA0)
            {
                ret = apduh_gsm_rcrd_update(swicc_state, cmd, res,
                                            procedure_count);
            }
            break;
