...
# Node server rather than python's: it also hosts /wapgw, the endpoint the page
# calls to fetch and transcode real sites for the phone's WAP browser (a page
# cannot fetch third-party sites itself). Falls back to python if node is gone,
# in which case the phone gets the gateway's built-in deck.
serve: all fw-manifest
	@echo "Serving on http://localhost:8000  (Ctrl-C to stop)"
	@command -v node >/dev/null && node tools/webserve.mjs 8000 \
